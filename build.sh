...
sudo docker build -t $REG:$VERSION .
sudo docker tag $REG:$VERSION $REG:$RELEASE
sudo docker push $REG:$RELEASE
sudo docker push $REG:$VERSION
...
If not, type <CTRL>+<C>"
read x
sudo docker tag $REG:$VERSION $REG:stable
sudo docker push $REG:stable
